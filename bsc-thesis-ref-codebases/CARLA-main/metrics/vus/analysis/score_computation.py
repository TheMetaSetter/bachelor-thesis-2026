        ...
        if label[i] == 1:
            if current_subseq == False:
                ...
                if best_i is None:
                    ...
                    best_i = i
                    best_sum = np.sum(
                        label[max(0, i - 200) : min(len(label), i + 9800)]
                    )
                else:
                    if (
                        np.sum(label[max(0, i - 200) : min(len(label), i + 9800)])
                        < best_sum
                    ):
                        changed = True
                        best_i = i
...
def compute_score(
    methods, slidingWindow, data, X_data, data_train, data_test, X_train, X_test
):
    methods_scores = {}

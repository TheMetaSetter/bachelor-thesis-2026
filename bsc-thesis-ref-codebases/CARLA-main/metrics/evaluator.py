...
from src.evaluation.evaluation_utils import (
    ...
    combine_entities_eval_metrics,
    get_dynamic_scores,
    get_gaussian_kernel_scores,
)
...
def evaluate(
    saved_model_root,
    logger,
    thres_methods=["top_k_time", "best_f1_test"],
    eval_root_cause=True,
    point_adjust=False,
    eval_R_model=True,
    eval_dyn=False,
    thres_config=None,
    telem_only=True,
    make_plots=["prc", "score_t"],
    ...
    for folder_name in saved_model_folders:
        ...
        plots_name = os.path.join(
            plots_dir, algo_name + "_" + me_ds_name + "_" + entity + "_"
        )
        # ds_name = dataset.name
        logger.info(
            "Processing Folder name: {}, {} on me dataset {}, entity {}".format(
                folder_name, algo_name, me_ds_name, entity
            )
        )
        ...
        try:
            ...
            logger.info(
                "The raw predictions of %s on %s weren't found, this run can't be evaluated"
                % (algo_name, me_ds_name)
            )
            return None
        ...
        rca_possible = (
            eval_root_cause
            and (
                preds["score_tc_test"] is not None or preds["error_tc_test"] is not None
            )
        ...
        eval_R = eval_R_model and (
            error_tc_test is not None or error_t_test is not None
        )
        eval_dyn = eval_dyn and (
            (error_tc_test is not None) or (error_t_test is not None)
        )

        # Evaluate on each entity
        logger.info("Evaluating for score_t")
        algo_results = collect_eval_metrics(
            algo_results=algo_results,
            score_t_test=score_t_test,
            y_test=y_test,
        ...
        if eval_R:
            if algo_name == "TelemanomAlgo":
                ...
                distr_par_file = os.path.join(
                    saved_model_root, folder_name, "distr_parameters"
                )
                if error_t_train is None or error_tc_train is None:
                    ...
                    distr_params = fit_distributions(
                        distr_par_file,
                        distr_names,
                        predictions_dic={"train_raw_scores": error_tc_train},
                    )[distr_names[0]]
                    (
                        score_t_train_gauss_s,
                        _,
                        score_t_test_gauss_s,
                        score_tc_train_gauss_s,
                        _,
                        score_tc_test_gauss_s,
                    ) = get_scores_channelwise(
                        distr_params,
                        train_raw_scores=error_tc_train,
                        val_raw_scores=None,
                        test_raw_scores=error_tc_test,
                        logcdf=True,
                    )

                telemanom_gauss_s_results = collect_eval_metrics(
                    algo_results=telemanom_gauss_s_results,
            ...
            if error_tc_train is not None and error_tc_test is not None:
                ...
                error_t_train_normed = np.sqrt(
                    np.mean(error_tc_train_normed**2, axis=1)
                )
            ...
            algo_R_results = collect_eval_metrics(
                algo_results=algo_R_results,
                score_t_test=error_t_test_normed,
                y_test=y_test,
                thres_methods=thres_methods,
                logger=logger,
                true_events=true_events,
                rca_possible=rca_possible,
                score_tc_test=error_tc_test_normed,
                root_causes=root_causes,
                score_t_train=error_t_train_normed,
                point_adjust=point_adjust,
                thres_config_dict=thres_config_dict,
                eval_methods=eval_methods,
                make_plots=make_plots,
        ...
        if eval_dyn:
            ...
            logger.info(
                "Evaluating gaussian dynamic scoring for error_t with thres_methods {}".format(
                    dyn_thres_methods
                )
            )
            ...
            algo_dyn_results = collect_eval_metrics(
                algo_results=algo_dyn_results,
                score_t_test=score_t_test_dyn,
                y_test=y_test,
                thres_methods=dyn_thres_methods,
                logger=logger,
                rca_possible=rca_possible,
                true_events=true_events,
                score_tc_test=score_tc_test_dyn,
                root_causes=root_causes,
                score_t_train=score_t_train_dyn,
                point_adjust=point_adjust,
                thres_config_dict=thres_config_dict,
                eval_methods=eval_methods,
                make_plots=make_plots,
            ...
            score_t_test_dyn_gauss_conv, score_tc_test_dyn_gauss_conv = (
                get_gaussian_kernel_scores(
                    score_t_test_dyn, score_tc_test_dyn, kernel_sigma
                )
            )
            if score_t_train_dyn is not None:
                score_t_train_dyn_gauss_conv, _ = get_gaussian_kernel_scores(
                    score_t_train_dyn, score_tc_train_dyn, kernel_sigma
                )
            else:
                score_t_train_dyn_gauss_conv = None
            algo_dyn_gauss_conv_results = collect_eval_metrics(
                algo_results=algo_dyn_gauss_conv_results,
    ...
    final_results, column_names = combine_entities_eval_metrics(
        algo_results,
        thres_methods,
        me_ds_name,
        algo_name,
    ...
    if eval_R_model:
        results_R, _ = combine_entities_eval_metrics(
            algo_R_results,
            thres_methods,
            me_ds_name,
            algo_name + "-R",
        ...
        if algo_name == "TelemanomAlgo":
            results_telem, _ = combine_entities_eval_metrics(
                telemanom_gauss_s_results,
                thres_methods,
                me_ds_name,
                algo_name + "-Gauss-S",
                rca_possible,
    ...
    if eval_dyn:
        results_dyn, _ = combine_entities_eval_metrics(
            algo_dyn_results,
            dyn_thres_methods,
            me_ds_name,
            algo_name + "-dyn",
        ...
        results_dyn_gauss_conv, _ = combine_entities_eval_metrics(
            algo_dyn_gauss_conv_results,
            dyn_thres_methods,
            me_ds_name,
            algo_name + "-dyn-gauss-conv",
            rca_possible,
            eval_methods=eval_methods,
        )
    ...
    new_col_order = (
        list(results_df.columns)[:3] + ["point_adjust"] + list(results_df.columns)[3:]
    )
...
def analyse_from_pkls(
    results_root: str,
    thres_methods=["best_f1_test"],
    eval_root_cause=True,
    point_adjust=False,
    eval_R_model=True,
    eval_dyn=True,
    ...
    for ds_folder in ds_folders:
        if (
            ds_folder.endswith(".csv")
            or ds_folder == "logs"
            or "thres_results" in ds_folder
        ):
            continue
        ...
        for algo_folder in algo_folders:
            ...
            config_folders = [
                folder for folder in config_folders if not folder.endswith(".csv")
            ]
                    ...
                    if not run_folder.endswith(".json"):
                        ...
                        if rerun_if_ds is not None:
                            if (ds_folder in rerun_if_ds) or (rerun_if_ds == "all"):
                                if os.path.exists(
                                    os.path.join(run_path, result_filename)
                                ):
                        ...
                        if not os.path.exists(os.path.join(run_path, result_filename)):
                            ...
                            for entity_folder in entity_folders:
                                if (
                                    entity_folder != "plots"
                                    and ".csv" not in entity_folder
                                    and entity_folder != "logs"
                                ):
                                    ...
                                    if not skip_this_run:
                                        try:
                                            with open(
                                                os.path.join(
                                                    entity_path, "raw_predictions"
                                                ),
                                                "rb",
                                            ) as file:
                                                raw_predictions = pickle.load(file)
                                            assert (
                                                "score_t_test" in raw_predictions.keys()
                                            )
                                            if np.isnan(
                                                raw_predictions["score_t_test"]
                                            ).any():
                                                skip_this_run = True
                                        except:
                                            ...
                                            repredict = repredict_from_saved_model(
                                                entity_path,
                                            ...
                                            if not repredict:
                                                logger.warning(
                                                    "Predictions and trained model couldn't be found, evaluation is "
                                                    "impossible for run saved at %s"
                                                    % run_path
                                                )
                                                skip_this_run = True
                            if not skip_this_run:
                                evaluate(
                                    run_path,
                                    thres_methods=thres_methods,
                                    eval_root_cause=eval_root_cause,
                                    point_adjust=point_adjust,
                                    eval_R_model=eval_R_model,
                                    eval_dyn=eval_dyn,
                                    thres_config=thres_config,
                                    logger=logger,
                        ...
                        try:
                            ...
                            logger.warning(
                                "Results table couldn't be found for run saved at %s"
                                % run_path
                            )
    overall_results = pd.concat(result_df_list, ignore_index=True)

    overall_results.to_csv(
        os.path.join(results_root, filename_prefix + "overall_" + result_filename)
    )
...
def repredict_from_saved_model(model_root, algo_class, entity, logger):
    ...
    try:
        algo_reload = load_torch_algo(
            algo_class,
            algo_config_filename,
            saved_model_filename,
        ...
        logger.warning(
            f"An error occurred while loading saved algo and repredicting: {e}"
        )
        return False

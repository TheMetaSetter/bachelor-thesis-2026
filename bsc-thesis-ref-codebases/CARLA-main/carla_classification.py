...
def main():
    ...
    if p["train_db_name"] == "MSL" or p["train_db_name"] == "SMAP":
        if p["fname"] == "All":
            with open(
                os.path.join(MyPath.db_root_dir("msl"), "labeled_anomalies.csv"), "r"
            ) as file:
            ...
            for file_name in data_info["chan_id"]:
                ...
                if ii == 0:
                    base_dataset = get_train_dataset(
                        p, train_transformations, sanomaly, to_neighbors_dataset=True
                    )
                    ...
                    new_base_dataset = get_train_dataset(
                        p, train_transformations, sanomaly, to_neighbors_dataset=True
                    )
                    new_val_dataset = get_val_dataset(
                        p,
                        val_transformations,
                        sanomaly,
                        True,
                        new_base_dataset.mean,
            ...
            info_ds = get_train_dataset(
                p, train_transformations, sanomaly, to_neighbors_dataset=False
            )
            val_dataset = get_val_dataset(
                p, val_transformations, sanomaly, False, info_ds.mean, info_ds.std
            )

    elif p["train_db_name"] == "yahoo":
        filename = os.path.join(
            "/home/zahraz/hz18_scratch/zahraz/datasets/", "Yahoo/", p["fname"]
        )
        dataset = []
        # print(filename)
        df = pandas.read_csv(filename)
        dataset.append(
            {"value": df["value"].tolist(), "label": df["is_anomaly"].tolist()}
        )
        ...
        p["train_db_name"] == "smd"
        or p["train_db_name"] == "kpi"
        or p["train_db_name"] == "swat"
        or p["train_db_name"] == "swan"
        or p["train_db_name"] == "gecco"
        or p["train_db_name"] == "wadi"
        or p["train_db_name"] == "ucr"
    ):
        base_dataset = get_train_dataset(
    ...
    if p["update_cluster_head_only"]:
        print(
            colored("WARNING: classification will only update the cluster head", "red")
        )

...
class NoiseTransformation(object):
    ...
    def __call__(self, X):
        ...
        noise = np.random.normal(
            loc=0, scale=self.sigma, size=X.shape
        )  # NumPy operation

        return torch.tensor(
            X.numpy() + noise, dtype=torch.float32, device=device
        )  # Move back to GPU
...
class SubAnomaly(object):
    ...
    def inject_frequency_anomaly(
        ...
        )  # cuda! PyTorch equivalent of np.tile()
        ...
        if shapelet_factor:
            ...
            anomalous_subsequence = window[start_index] + (
                torch.rand_like(window[start_index]) * 0.1
            )  # cuda use!
    ...
    def __call__(self, X):
        ...
        if window.ndim > 1:
            ...
            num_dims = np.random.randint(
                int(num_features / 10), int(num_features / 2)
            )  # (int(num_features/5), int(num_features/2))

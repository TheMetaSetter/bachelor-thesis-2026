...
def pretext_train(
    train_loader, model, criterion, optimizer, epoch, prev_loss, device="cuda"
):
    losses = AverageMeter("Loss", ":.4e")
    progress = ProgressMeter(
        len(train_loader), [losses], prefix="Epoch: [{}]".format(epoch + 1)
    )
    ...
    for i, batch in enumerate(train_loader):
        # ts_org is the anchor sample
        # ts_w_augmented is the positive sample (neighbor sample)
        # ts_ss_augmented is the negative sample (anomaly injected sample)
...
def self_sup_classification_train(
    train_loader, model, criterion, optimizer, epoch, update_cluster_head_only=False
):
    """
    ...
    for i, batch in enumerate(train_loader):
        ...
        for (
            ...
            nneighbors_output_subhead,
            fneighbors_output_subhead,
        ) in zip(anchors_output, nneighbors_output, fneighbors_output):
            total_loss_, consistency_loss_, inconsistency_loss_, entropy_loss_ = (
                criterion(
                    anchors_output_subhead,
                    nneighbors_output_subhead,
                    fneighbors_output_subhead,
        ...
        consistency_loss_values = [
            v.item() for v in consistency_loss if not torch.isnan(v)
        ]
        inconsistency_loss_values = [
            v.item() for v in inconsistency_loss if not torch.isnan(v)
        ]

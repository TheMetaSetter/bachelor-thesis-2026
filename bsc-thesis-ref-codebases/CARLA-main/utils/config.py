...
def create_config(config_file_env, config_file_exp, fname):
    ...
    cfg["bottomk_neighbors_train_path"] = os.path.join(
        pretext_dir, "bottomk-train-neighbors.npy"
    )
    cfg["aug_train_dataset"] = os.path.join(pretext_dir, "aug_train_dataset.pth")
    cfg["pretext_features_train_path"] = os.path.join(
        pretext_dir, "pretext_features_train.npy"
    )
    cfg["pretext_features_test_path"] = os.path.join(
        pretext_dir, "pretext_features_test.npy"
    )
    cfg["topk_neighbors_val_path"] = os.path.join(
        pretext_dir, "topk-test-neighbors.npy"
    )
    cfg["bottomk_neighbors_val_path"] = os.path.join(
        pretext_dir, "bottomk-test-neighbors.npy"
    )
    cfg["bottomk_neighbors_val_path"] = os.path.join(
        pretext_dir, "bottomk-test-neighbors.npy"
    )

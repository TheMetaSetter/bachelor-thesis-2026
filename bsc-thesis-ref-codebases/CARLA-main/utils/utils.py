...
def fill_ts_repository(
    p, loader, model, ts_repository, real_aug=False, ts_repository_aug=None
):
    model.eval()
    ts_repository.reset()
    ...
    for i, batch in enumerate(loader):
        ...
        if real_aug:
            ...
            targets = torch.LongTensor([2] * ts_w_augment.shape[0]).to(
            ...
            targets = torch.LongTensor([4] * ts_ss_augment.shape[0]).to(
                device, non_blocking=True
            )

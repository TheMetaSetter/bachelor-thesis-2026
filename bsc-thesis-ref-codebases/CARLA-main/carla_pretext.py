...
def main():
    ...
    if p["train_db_name"] == "MSL" or p["train_db_name"] == "SMAP":
        if p["fname"] == "All":
            ...
            for file_name in data_info["chan_id"]:
                ...
                if ii == 0:
                    ...
                    new_train_dataset = get_train_dataset(
                        p,
                        train_transforms,
                        sanomaly,
                        to_augmented_dataset=True,
        ...
        print(
            ">>>",
            "train/test w. shapes of {}/{}".format(
                np.shape(TRAIN_TS), np.shape(TEST_TS)
            ),
        )
    ...
    if os.path.exists(p["pretext_checkpoint"]):
        print(
            colored(
                "Restart from checkpoint {}".format(p["pretext_checkpoint"]), "blue"
            )
        )
        ...
        print(
            colored("No checkpoint file at {}".format(p["pretext_checkpoint"]), "blue")
        )
        start_epoch = 0
        model = model.to(device)
    ...
    print(
        colored(
            "Fill TS Repository for mining the nearest/furthest neighbors (val) ...",
            "blue",
        )
    )

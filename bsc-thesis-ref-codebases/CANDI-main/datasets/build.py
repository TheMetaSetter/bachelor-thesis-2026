...
class TSDataset(Dataset):
    ...
    def _save_downsampled_data(
        self, train, val, test, train_labels, val_labels, test_labels
    ):
        assert self.downsample_rate > 1
        np.save(
            os.path.join(
                self.data_dir, f"downsampled_train_rate{self.downsample_rate}.npy"
            ),
            train,
        )
        np.save(
            os.path.join(
                self.data_dir, f"downsampled_val_rate{self.downsample_rate}.npy"
            ),
            val,
        )
        np.save(
            os.path.join(
                self.data_dir, f"downsampled_test_rate{self.downsample_rate}.npy"
            ),
            test,
        )
        np.save(
            os.path.join(
                self.data_dir,
                f"downsampled_train_labels_rate{self.downsample_rate}.npy",
            ),
            train_labels,
        )
        np.save(
            os.path.join(
                self.data_dir, f"downsampled_val_labels_rate{self.downsample_rate}.npy"
            ),
            val_labels,
        )
        np.save(
            os.path.join(
                self.data_dir, f"downsampled_test_labels_rate{self.downsample_rate}.npy"
            ),
            test_labels,
        )
    ...
    def print_data_stats(self):
        if self.split == "train":
            print(
                f"Train data shape: {self.train.shape}, mean: {np.mean(self.train, axis=0)}, std: {np.std(self.train, axis=0)}"
            )
        elif self.split == "val":
            print(
                f"Validation data shape: {self.val.shape}, mean: {np.mean(self.val, axis=0)}, std: {np.std(self.val, axis=0)}"
            )
        elif self.split == "test":
            print(
                f"Test data shape: {self.test.shape}, mean: {np.mean(self.test, axis=0)}, std: {np.std(self.test, axis=0)}"
            )
    ...
    def __getitem__(self, index):
        ...
        return np.float32(inputs[index : index + self.win_size]), int(
            np.any(labels[index : index + self.win_size] == 1)
        )
...
class SMDSegLoader(TSDataset):
    ...
    def _load_data(self):
        ...
        with open(
            os.path.join(
                self.data_dir, "preprocessed", f"machine-{self.entity}_train.pkl"
            ),
            "rb",
        ) as f:
            train = pickle.load(f)
        with open(
            os.path.join(
                self.data_dir, "preprocessed", f"machine-{self.entity}_test.pkl"
            ),
            "rb",
        ) as f:
            test = pickle.load(f)
        with open(
            os.path.join(
                self.data_dir, "preprocessed", f"machine-{self.entity}_test_label.pkl"
            ),
            "rb",
        ) as f:
        ...
        if self.train_ratio < 1.0:
            train, val, train_labels, val_labels = self._split_train_val(
                train, train_labels
            )
        else:
    ...
    def _load_data(self):
        if self.downsample_rate > 1:
            try:
                ...
                print(
                    f"downsampled data not found. Load original data instead and downsample."
                )
        ...
        if os.path.exists(train_csv_path):
            ...
            raise FileNotFoundError(
                f"Neither {train_csv_path} nor {train_excel_path} exists."
            )
        ...
        if os.path.exists(test_csv_path):
            ...
            raise FileNotFoundError(
                f"Neither {test_csv_path} nor {test_excel_path} exists."
            )
        ...
        if self.downsample_rate > 1:
            train, val, test = (
                downsample(train, self.downsample_rate),
                downsample(val, self.downsample_rate),
                downsample(test, self.downsample_rate),
            )
            train_labels, val_labels, test_labels = (
                downsample(train_labels, self.downsample_rate),
                downsample(val_labels, self.downsample_rate),
                downsample(test_labels, self.downsample_rate),
            )
            self._save_downsampled_data(

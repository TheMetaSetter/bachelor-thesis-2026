...
class POLY(BaseDetector):
    ...
    def __init__(
        self, power=1, window=200, neighborhood=None, contamination=0.1, normalize=True
    ):
        self.power = power
        self.window = window
    ...
    def fit(self, X, y=None):
        ...
        for i in range(M, N + 1):
            ...
            if index + neighbor < self.n_train_ and index - neighbor > 0:
                x = np.concatenate(
                    (
                        np.arange(index - neighbor, index),
                        np.arange(index + window, index + neighbor),
                    )
                )
                y = np.concatenate(
                    (
                ...
                x = np.concatenate(
                    (
                        np.arange(self.n_train_ - neighborhood, index),
                        np.arange(index + window, self.n_train_),
                    )
                )
                y = np.concatenate(
                    (
                ...
                x = np.concatenate(
                    (np.arange(0, index), np.arange(index + window, neighborhood))
                )
                ...
                try:
                    ...
                    x = np.concatenate(
                        (
                            np.arange(0, index),
                            np.arange(len(data[index + window : neighborhood])),
                        )
                    )
                    mymodel = np.poly1d(np.polyfit(x, y, power))
    ...
    def decision_function(self, X=False, measure=None):
        ...
        for i in range(M, N + 1):
            ...
            score[index : index + window] = measure.measure(
                self.X_train_[index:end], estimation[index:end], index
            )

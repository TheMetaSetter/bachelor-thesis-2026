...
class CNNModel(nn.Module):
    def __init__(
        ...
        for idx, out_channels in enumerate(self.num_channel[:-1]):
            self.conv_layers.add_module(
                ...
                torch.nn.Conv1d(
                    prev_channels,
                    self.num_channel[idx + 1],
                    self.kernel_size,
                    self.stride,
                ),
            )
            self.conv_layers.add_module(
                self.hidden_activation + str(idx), self.activation
            )
    ...
    def forward(self, x):
        ...
        outputs = torch.zeros(self.predict_time_steps, b, self.n_features).to(
            self.device
        )
...
class CNN:
    ...
    def fit(self, data):
        ...
        train_loader = DataLoader(
            ForecastDataset(
                tsTrain, window_size=self.window_size, pred_len=self.pred_len
            ),
        ...
        valid_loader = DataLoader(
            ForecastDataset(
                tsValid, window_size=self.window_size, pred_len=self.pred_len
            ),
            batch_size=self.batch_size,
            shuffle=False,
        )
    ...
    def param_statistic(self, save_file):
        model_stats = torchinfo.summary(
            self.model, (self.batch_size, self.window_size), verbose=0
        )

    ...
    def zero_shot(self, data):
        ...
        with torch.no_grad():
            for batch_x, batch_masks in tqdm(test_loader, total=len(test_loader)):
                ...
                score = (
                    torch.mean(
                        self.anomaly_criterion(batch_x, output.reconstruction), dim=-1
                    )
                    .detach()
                    .cpu()
                    .numpy()[:, -1]
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for batch_x, batch_masks in tqdm(train_loader, total=len(train_loader)):
                ...
                mask = torch.nn.functional.pad(
                    mask,
                    (0, batch_masks.size(1) - mask.size(1)),
                    mode="constant",
                    value=1,
                )
                ...
                model_output = self.model(
                    batch_x, input_mask=batch_masks, mask=mask
                ).reconstruction
                model_output = torch.nn.functional.pad(
                    model_output,
                    (0, original.size(2) - model_output.size(2)),
                    mode="replicate",
                )

                output = model_output.reshape(
                    original.size(0), n_channels, self.win_size
                )

...
class OFA:
    def __init__(
        ...
        self.model = (
            Model(seq_len=self.win_size, enc_in=self.enc_in, c_out=self.enc_in)
            .float()
            .to(self.device)
        )
    ...
    def fit(self, data):
        ...
        train_loader = DataLoader(
            dataset=ReconstructDataset(
                tsTrain, window_size=self.win_size, stride=self.stride
            ),
        ...
        valid_loader = DataLoader(
            dataset=ReconstructDataset(
                tsValid, window_size=self.win_size, stride=self.stride
            ),
            batch_size=self.batch_size,
            shuffle=False,
        )
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            adjust_learning_rate(
                self.model_optim, epoch + 1, self.lradj, self.learning_rate
            )

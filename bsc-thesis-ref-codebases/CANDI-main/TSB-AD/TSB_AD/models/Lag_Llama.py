    ...
    def fit(self, data):
        for channel in range(self.input_c):
            ...
            date_rng = pd.date_range(
                start="2021-01-01", periods=data_win.shape[0], freq="H"
            )  # Dummy timestep
            ...
            forecast_it, ts_it = make_evaluation_predictions(
                dataset=ds, predictor=predictor, num_samples=self.num_samples
            )
        ...
        padded_decision_scores[: self.context_length + self.prediction_length - 1] = (
            scores_merge[0]
        )
        padded_decision_scores[self.context_length + self.prediction_length - 1 :] = (
            scores_merge

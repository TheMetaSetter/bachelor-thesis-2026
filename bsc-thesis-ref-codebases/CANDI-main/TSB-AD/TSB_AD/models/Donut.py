...
def normal_standard_normal_kl(
    mean: torch.Tensor, std_or_log_var: torch.Tensor, log_var: bool = False
) -> torch.Tensor:
    if log_var:
        kl_loss = torch.sum(
            1 + std_or_log_var - mean.pow(2) - std_or_log_var.exp(), dim=-1
        )
    else:
        kl_loss = torch.sum(
            1 + torch.log(std_or_log_var.pow(2)) - mean.pow(2) - std_or_log_var.pow(2),
            dim=-1,
        )
    return -0.5 * kl_loss


def normal_normal_kl(
    mean_1: torch.Tensor,
    ...
    if log_var:
        return 0.5 * torch.sum(
            std_or_log_var_2
            - std_or_log_var_1
            + (torch.exp(std_or_log_var_1) + (mean_1 - mean_2) ** 2)
            / torch.exp(std_or_log_var_2)
            - 1,
            dim=-1,
        )

    return torch.sum(
        torch.log(std_or_log_var_2)
        - torch.log(std_or_log_var_1)
        + 0.5 * (std_or_log_var_1**2 + (mean_1 - mean_2) ** 2) / std_or_log_var_2**2
        - 0.5,
        dim=-1,
    )
...
class VAELoss(torch.nn.modules.loss._Loss):
    ...
    def forward(
        ...
        **kwargs,
        ...
        nll_gauss = F.gaussian_nll_loss(
            x_dec_mean, y, x_dec_std.pow(2), reduction="none"
        ).sum(-1)
        ...
        if z_prior_mean is None and z_prior_std_or_logvar is None:
            ...
            kl_loss = normal_standard_normal_kl(
                z_mean, z_std_or_log_var, log_var=self.logvar_out
            )
        else:
            ...
            if z_prior_std_or_logvar is None:
                ...
                z_prior_std_or_logvar = torch.tensor(
                    value, dtype=z_std_or_log_var.dtype, device=z_std_or_log_var.device
                )

            kl_loss = normal_normal_kl(
                z_mean,
                z_std_or_log_var,
                z_prior_mean,
                z_prior_std_or_logvar,
...
class MaskedVAELoss(VAELoss):
    def __init__(self, size_average=None, reduce=None, reduction: str = "mean"):
        super(MaskedVAELoss, self).__init__(
            size_average, reduce, reduction, logvar_out=False
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if mask is None:
            ...
            return super(MaskedVAELoss, self).forward(
                (mean_z, std_z, mean_x, std_x), (actual_x,), *args, **kwargs
            )
        ...
        nll_output = torch.sum(
            mask * F.gaussian_nll_loss(mean_x, actual_x, std_x**2, reduction="none"),
            dim=-1,
        )
        ...
        nll_approx = torch.sum(
            F.gaussian_nll_loss(mean_z, sample_z, std_z**2, reduction="none"),
            dim=-1,
            keepdim=True,
        )
...
class MLP(torch.nn.Module):
    def __init__(
        ...
        self.layers = torch.nn.ModuleList(
            [torch.nn.Linear(inp, out) for inp, out in zip(layers[:-1], layers[1:])]
        )
...
class VAE(torch.nn.Module):
    ...
    def forward(
        ...
        if self.training or num_samples > 1 or force_sample:
            z_sample = sample_normal(
                z_mu, z_std_or_log_var, log_var=self.log_var, num_samples=num_samples
            )
        else:
            z_sample = z_mu
...
class Donut:
    def __init__(
        ...
        self.model = DonutModel(
            ...
            mask_prob=inject_ratio,
        ).to(self.device)
        self.optimizer = optim.AdamW(
            self.model.parameters(), lr=lr, weight_decay=l2_coff
        )
    ...
    def decision_function(self, data):
        ...
        with torch.no_grad():
            for idx, (x, _) in loop:
                ...
                res = self.model.vae(
                    x_vae, return_latent_sample=False, num_samples=self.mc_samples
                )
    ...
    def param_statistic(self, save_file):
        model_stats = torchinfo.summary(
            self.model, (self.batch_size, self.win_size), verbose=0
        )

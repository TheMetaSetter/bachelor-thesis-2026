...
class SANA(nn.Module):
    ...
    def forward(self, x):
        ...
        dec_out = torch.stack(
            [proj(enc_out[:, i, :]) for i, proj in enumerate(self.projection)], dim=1
        )  # (B, N, L)
...
class MLP(nn.Module):
    ...
    def forward(self, X):
        ...
        if hasattr(self, "sana_in") and hasattr(self, "sana_out"):
            ...
            reconstructed_x_new = self.decoder(
                self.encoder(x_new.reshape(B, L * C))
            ).reshape(B, L, C)
            reconstructed_x = reconstructed_x_new - self.sana_out(reconstructed_x_new)
        else:
            reconstructed_x = self.decoder(self.encoder(X.reshape(B, L * C))).reshape(
                B, L, C
            )
...
class MLP_Tester:
    ...
    def online(self, dataloader, init_thr):
        ...
        topk_indices_tensor = torch.from_numpy(topk_indices).to(
            val_representations.device
        )
        ...
        moderate_indices_tensor = torch.from_numpy(moderate_indices).to(
            val_representations.device
        )
        ...
        self.model.sana_in = SANA(
            self.model.L, self.model.C, gating_init=self.gating_init
        ).to(self.device)
        self.model.sana_out = SANA(
            self.model.L, self.model.C, gating_init=self.gating_init
        ).to(self.device)
            ...
            with torch.no_grad():
                ...
                mahalanobis_dist_square_moderate = torch.sum(
                    (
                        ...
                        - moderate_representations.unsqueeze(0)
                    )
                    @ representations_cov_inv
                    * (
                        representations.unsqueeze(1)
...
class CANDI(BaseDetector):
    ...
    def decision_function(self, data):
        ...
        if self.infer_mode == "online":
            anoscs = self.tester.online(self.test_loader, self.tau)
        else:

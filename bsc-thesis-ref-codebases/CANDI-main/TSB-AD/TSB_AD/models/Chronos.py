    ...
    def fit(self, data):
        for channel in range(self.input_c):
            ...
            train_data = pd.DataFrame(
                train_data, columns=["item_id", "timestamp", "target"]
            )

            with tempfile.TemporaryDirectory() as temp_dir:
                predictor = TimeSeriesPredictor(
                    prediction_length=self.prediction_length, path=temp_dir
                ).fit(
                    train_data,
                    hyperparameters={
                        "Chronos": {
                ...
                predictions = (
                    predictor.predict(train_data)["mean"]
                    .to_numpy()
        ...
        padded_decision_scores[: self.win_size + self.prediction_length - 1] = (
            scores_merge[0]
        )
        padded_decision_scores[self.win_size + self.prediction_length - 1 :] = (
            scores_merge

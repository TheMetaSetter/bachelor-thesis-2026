...
class SANA(nn.Module):
    def __init__(self, cfg):
        ...
        self.gating = nn.Parameter(
            self.cfg.TEST.TTA.CANDI.GATING_INIT * torch.ones(self.n_var)
        )
        ...
        if self.cfg.SANA.TYPE == "TCN_iTrans":
            ...
            self.projection = nn.ModuleList(
                [
                    nn.Linear(self.cfg_model.d_model, self.cfg.DATA.WIN_SIZE)
            ...
            self.projection = nn.ModuleList(
                [
                    nn.Linear(self.cfg.DATA.WIN_SIZE, self.cfg.DATA.WIN_SIZE)
                    for _ in range(self.n_var)
                ]
            )
        else:
    ...
    def forward(self, x):
        ...
        if self.cfg.SANA.TYPE == "TCN_iTrans":
            enc_out = torch.stack(
                [
                    embed(x[:, i : i + 1, :])
                    for i, embed in enumerate(self.temporal_embedding)
                ],
                dim=1,
            )  # (B, N, D)
            ...
            dec_out = torch.stack(
                [proj(enc_out[:, i, :]) for i, proj in enumerate(self.projection)],
                dim=1,
            )  # (B, N, L)
        elif self.cfg.SANA.TYPE == "Linear":
            dec_out = torch.concat(
                [embed(x[:, i : i + 1, :]) for i, embed in enumerate(self.projection)],
                dim=1,
            )
        else:
            raise ValueError(f"Unsupported SANA.TYPE: {self.cfg.SANA.TYPE}")
...
class CANDIAdapter(Adapter):
    def __init__(self, cfg, model, thresholder, **kwargs):
        ...
        self.topk = int(
            len(self.val_scores) * self.cfg.TEST.THRESHOLD.ANOMALY_RATIO / 100
        )
    ...
    def get_samples_to_adapt(self, x, scores):
        if not self.cfg.TEST.TTA.CANDI.USE_FPM:
            ...
            selected_x_moderate = x[
                mask_moderate
            ]  # Handle as moderate for implementation convenience
            if len(selected_x_moderate) > 0:
                selected_indices_moderate = mask_moderate.nonzero(as_tuple=True)[0]
                offset = self.iter * len(scores)
                selected_indices_moderate_offset = (
                    selected_indices_moderate + offset
                ).tolist()

                selected_labels_moderate = self.test_labels[
                    selected_indices_moderate_offset
                ]
                ...
                self.total_anomalies_in_moderate += (
                    (selected_labels_moderate == 1).sum().item()
                )
            return
        ...
        if self.cfg.TEST.TTA.CANDI.USE_HARD:
            mahalanobis_dist_square_hard = torch.sum(
                ...
                * (
                    ...
                    - self.topk_representations.unsqueeze(0)
                ),
                dim=-1,
            )
        ...
        if self.cfg.TEST.TTA.CANDI.USE_MODERATE:
            ...
            mahalanobis_dist_square_moderate = torch.sum(
                (
                    representations.unsqueeze(1)
                    - self.moderate_representations.unsqueeze(0)
                )
                @ self.representations_cov_inv
                * (
                    representations.unsqueeze(1)
            ...
            mask_moderate_sim = (
                mahalanobis_dist_square_moderate < chi2_threshold_moderate
            )
    ...
    def adapt(self, x, scores):
        ...
        if self.n_samples_to_adapt_moderate >= self.cfg.TEST.TTA.CANDI.MIN_SAMPLES:
            self.n_adapt += 1
            self.samples_to_adapt_moderate = torch.concat(
                self.samples_to_adapt_moderate, dim=0
            )
            is_eval = not deepcopy(self.model.training)
            self.model.train()
    ...
    def calculate_loss(self, x):
        ...
        raise NotImplementedError(
            "The calculate_loss method should be implemented in subclasses."
        )

...
def get_mlp(
    ...
    if use_output_bn:
        layers.append(
            nn.BatchNorm1d(f[-1], affine=False)
        )  # no learnable parameters, like in SimSiam
    return nn.Sequential(*layers)


def get_backbone(
    args: argparse.Namespace, is_target: bool = False
) -> Tuple[nn.Module, int]:
    if args.arch in vits.__dict__.keys():
        if args.drop_path_rate > 0.0 and not is_target:
            print(f"Using {args.drop_path_rate} drop path rate.")
        drop_path_rate = (
            0.0 if is_target else args.drop_path_rate
        )  # stochastic depth only for the online network
        ...
        if args.use_jax_resnet:
            warnings.warn(
                "Using JAX-like ResNet implementation (only meant for evaluation of JAX pre-trained models)."
            )
...
class BYOLNetwork(nn.Module):
    ...
    def __init__(self, args: argparse.Namespace, is_target: bool = False):
        ...
        self.projector = get_mlp(
            args.projector_mlp,
            self.embedding_dim,
            activation=args.projector_activ,
            use_output_bn=args.use_proj_output_bn,
        )
        self.predictor = get_mlp(
            args.predictor_mlp,
            self.num_features,
            activation=args.predictor_activ,
...
class BYOLChimeraNetwork(BYOLNetwork):
    ...
    def __init__(self, args: argparse.Namespace, is_target: bool = False):
        ...
        }  # dummy predictor when no prediction is needed
        for task in args.tasks:
            predictors[task] = get_mlp(
                args.predictor_mlp, self.num_features, activation=args.predictor_activ
            )

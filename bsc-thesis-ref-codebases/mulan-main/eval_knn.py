...
def extract_features(
    model, data_loader, use_cuda: bool = True, distributed: bool = False
):
    features = None
    for samples, index in tqdm(data_loader, desc="Extracting features"):
        ...
        if distributed:
            y_all = torch.empty(
                get_world_size(), index.size(0), dtype=index.dtype, device=index.device
            )
...
def knn_classifier(
    ...
    assert similarity_type in ["euclidean", "cosine"], (
        "similarity_type must be either 'euclidean' or 'cosine'"
    )
    ...
    all_indices = torch.zeros(num_test_images, k, dtype=torch.int32).to(
    ...
    for idx in range(0, num_test_images, imgs_per_chunk):
        ...
        if similarity_type == "cosine":
            ...
            similarity = -torch.cdist(
                features.unsqueeze(0), train_features.unsqueeze(0), p=2
            ).squeeze(0)
...
def evaluate_knn(args, train_dataset: Optional[ReturnIndexDataset] = None):
    ...
    train_features = extract_features(
        backbone, train_loader, use_cuda=True, distributed=args.distributed
    )
    print("Extracting features for val set...")
    val_features = extract_features(
        backbone, val_loader, use_cuda=True, distributed=args.distributed
    )
    ...
    if is_main_process():
        ...
        acc1, acc5, topk_indices = knn_classifier(
            train_features,
            train_labels,
            val_features,
            val_labels,

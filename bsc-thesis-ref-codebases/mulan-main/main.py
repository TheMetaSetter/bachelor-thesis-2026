...
def get_arguments():
    parser = argparse.ArgumentParser(
        description="Pretrain a Siamese asymmetric SSL model", add_help=False
    )
    ...
    parser.add_argument(
        "--drop-path-rate", type=float, default=0.0, help="Drop path rate (for ViT)"
    )
    parser.add_argument(
        "--projector-mlp",
        default="4096-256",
...
def main(args):
    ...
    if args.distributed:
        ...
        print(
            f"Using distributed training with {args.world_size} workers, {per_device_batch_size = }"
        )
    else:
    ...
    if args.distributed:
        ...
        if not broadcast_buffers:
            warnings.warn(
                "Disabling DDP buffers broadcasting, may have a negative impact at EVAL."
            )
        online_model = torch.nn.parallel.DistributedDataParallel(
            ...
            find_unused_parameters=(
                "multitask" in args.training_fn and not args.single_pred_head
            ),  # needed for multitask
        )
        if args.sync_target_bn:
            ...
            target_model = torch.nn.parallel.DistributedDataParallel(
                target_model, device_ids=[args.gpu], broadcast_buffers=False
            )
    ...
    if args.compile:
        ...
        if not args.distributed:
            print(
                "Compiling online model with online_model.backbone.compile(mode='default')"
            )
            online_model.backbone.compile(mode="default")
        if not args.sync_target_bn:
            print(
                "Compiling target model with: target_model.backbone.compile(mode='max-autotune')"
            )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            metric_logger.add_meter(
                f"lr_group_{i}", utils.SmoothedValue(window_size=1, fmt="{value}")
            )
            metric_logger.add_meter(
                f"wd_group_{i}", utils.SmoothedValue(window_size=1, fmt="{value}")
            )
        ...
        for step, (images_tuple, _) in enumerate(
            ...
            if args.clip_grad > 0:
                ...
                grad_norm = torch.nn.utils.clip_grad_norm_(
                    online_model.parameters(), args.clip_grad
                )
            else:
        ...
        if is_main_process():
            ...
            if args.save_freq != -1 and (epoch + 1) % args.save_freq == 0:
                torch.save(
                    state, args.output_dir / f"checkpoint_epoch_{epoch + 1:0>4}.pth"
                )
    ...
    if is_main_process():
        torch.save(
            online_model.module.backbone.state_dict(), args.output_dir / "backbone.pth"
        )

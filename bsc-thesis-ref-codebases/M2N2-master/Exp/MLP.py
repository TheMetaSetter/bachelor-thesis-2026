...
class MLP_Trainer(Trainer):
    def __init__(self, args, logger, train_loader):
        ...
        self.optimizer = torch.optim.AdamW(
            params=self.model.parameters(), lr=args.lr, weight_decay=self.args.L2_reg
        )  # L2 Reg is set to zero by default, but can be set as needed.
        self.logger.info(f"\n{self.model}")
...
class MLP_Tester(Tester):
    def __init__(self, args, logger, train_loader, test_loader, load=False):
        super(MLP_Tester, self).__init__(
            args=args, logger=logger, train_loader=train_loader, test_loader=test_loader
        )
    ...
    def online(self, dataloader, init_thr, normalization="None"):
        ...
        it = tqdm(dataloader, total=len(dataloader), desc="inference", leave=True)

        tau = init_thr
        TT_optimizer = torch.optim.SGD(
            [p for p in self.model.parameters()], lr=self.args.ttlr
        )
        ...
        thrs = np.repeat(
            np.array(thrs), self.args.window_size * self.args.eval_batch_size
        )
        ...
        if self.args.plot_anomaly_scores:
            ...
            plt.plot(
                self.test_anoscs,
                color="blue",
                label="anomaly score w/o online learning",
            )
            ...
            plt.savefig(
                os.path.join(
                    self.args.plot_path, f"{self.args.exp_id}_wTTA_tau_{tau}.png"
                )
            )
        ...
        if self.args.plot_recon_status:
            ...
            for c in range(self.args.num_channels):
                ...
                plt.plot(
                    ...
                    linewidth=0.5,
                    color="green",
                    label="reconed w/ online learning",
                )
                plt.legend()
                plot_interval(plt, self.gt)

...
class USAD_Trainer(Trainer):
    ...
    def _process_batch(self, batch_data, epoch) -> dict:
        ...
        Wt2dp = self.model.decoder2(self.model.encoder(Wt1p.reshape(B, L * C))).reshape(
            B, L, C
        )

        self.optimizer1.zero_grad()
        loss_AE1 = (1 / epoch) * F.mse_loss(X, Wt1p) + (1 - (1 / epoch)) * F.mse_loss(
            X, Wt2dp
        )
        loss_AE1.backward()
        ...
        loss_AE2 = (1 / epoch) * F.mse_loss(X, Wt2p) - (1 - (1 / epoch)) * F.mse_loss(
            X, Wt2dp
        )
        loss_AE2.backward()
...
class USAD_Tester(Tester):
    ...
    def calculate_anomaly_scores(self, dataloader):
        ...
        anomaly_scores = (
            recon_errors.mean(dim=2).reshape(-1).detach().cpu()
        )  # B, L -> (T=B*L, )
    ...
    def calculate_recon_errors(self, dataloader):
        ...
        for i, batch_data in enumerate(eval_iterator):
            X = batch_data[0].to(self.args.device)
            recon_error = self.recon_error_criterion(
                X, self.args.model.alpha, self.args.model.beta
            )
    ...
    def recon_error_criterion(self, Wt, alpha=0.5, beta=0.5):
        ...
        return alpha * F.mse_loss(Wt, Wt1p, reduction="none") + beta * F.mse_loss(
            Wt, Wt2dp, reduction="none"
        )

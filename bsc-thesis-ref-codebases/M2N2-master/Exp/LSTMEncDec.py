...
class LSTMEncDec_Trainer(Trainer):
    def __init__(self, args, logger, train_loader):
        super(LSTMEncDec_Trainer, self).__init__(
...
class LSTMEncDec_Tester(Tester):
    def __init__(self, args, logger, train_loader, test_loader, load=False):
        super(LSTMEncDec_Tester, self).__init__(
    ...
    def calculate_anomaly_scores(self, dataloader):
        ...
        train_recon_errors = self.calculate_recon_errors(
            dataloader=self.train_loader
        )  # (B, L, C)
        B, L, C = train_recon_errors.shape
        train_recon_errors = train_recon_errors.reshape(B * L, C)
        train_error_mu, train_error_cov = (
            np.mean(train_recon_errors, axis=0),
            np.cov(train_recon_errors.T),
        )
        ...
        ic = (
            np.linalg.pinv(train_error_cov) if C > 1 else np.array([[train_error_cov]])
        )  # inverse of covariance matrix, (C, C)

...
class Tester:
    ...
    def prepare_stats(self):
        ...
        if self.args.load_anoscs and os.path.isfile(train_anoscs_pt_path):
            ...
            train_anoscs = torch.Tensor(
                self.calculate_anomaly_scores(self.train_loader)
            )  # (B, L, C) => (B, L)
        ...
        if self.args.load_anoscs and os.path.isfile(test_anosc_pt_path):
            ...
            test_anoscs = torch.Tensor(
                self.calculate_anomaly_scores(self.test_loader)
            )  # (B, L, C) => (B, L)
        ...
        self.train_anoscs = (
            train_anoscs.detach().cpu().numpy()[train_mask]
        )  # does not include -1's
        self.test_anoscs = (
            test_anoscs.detach().cpu().numpy()
        )  # may include -1's, filtered when calculating final results.
        ...
        self.th_off_f1_best = get_best_static_threshold(
            gt=self.gt, anomaly_scores=self.test_anoscs
        )
    ...
    def infer(self, mode, cols):
        ...
        if th[0] == "q":
            ...
            raise ValueError(
                f"Thresholding mode {self.args.thresholding} is not supported."
            )
        ...
        if mode == "offline":
            ...
            anoscs, pred = self.online(
                self.test_loader, tau, normalization=self.args.normalization
            )
            result = get_summary_stats(gt, pred)
            roc_auc = calculate_roc_auc(
                gt,
        ...
        if self.args.save_result:
            filename = (
                f"{self.args.exp_id}_{mode}_{th}"
                if (not hasattr(self.args, "qStart"))
            ...
            if len(result_df) == 1:
                result_dict = literal_eval(
                    result_df.reset_index(drop=True).to_json(orient="index")
                )["0"]
                # save as unique id
                with open(
                    os.path.join(self.args.result_path, filename + ".json"), "w"
                ) as f:
    ...
    def offline(self, tau):
        ...
        plt.plot(
            self.test_anoscs, color="blue", label="anomaly score w/o online learning"
        )
        ...
        plt.legend()
        plt.savefig(
            os.path.join(self.args.plot_path, f"{self.args.exp_id}_offline.png")
        )

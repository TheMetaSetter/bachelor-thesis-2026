...
def main(args):
    ...
    train_loader = DataLoader(
        args, data_train, gold_train, args.batch_size, word_to_int
    )
    ...
    if args.transformer:
        model = Model_T(
            ...
            args.n_layers,
            args.per_layer,
            args.dropout_prob_classifier,
            args.dropout_prob_attn,
            args.dropout_prob_hidden,
            args.use_elmo,
            args.num_rep,
            args.elmo_drop,
            args.gpu_id,
...
def inference(model, lossFunction, ood_dev_loader, args):
    ...
    if args.gpu_id > -1:
        model.load_state_dict(
            torch.load(args.model_path + "_" + args.model_name + ".pkl")
        )
    else:
        model.load_state_dict(
            torch.load(
                args.model_path + "_" + args.model_name + ".pkl", map_location="cpu"
            )
        )
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % eval_every == 0:
            ...
            if val_best < val_mcc:
                val_best = val_mcc
                torch.save(
                    model.state_dict(), args.model_path + "_" + args.model_name + ".pkl"
                )
...
def prediction(model, test_loader, args):
    ...
    tn, tp, fn, fp = 0, 0, 0, 0
    with open(
        "predictions/" + args.model_type + "_in_test_prediction.csv", "w", newline=""
    ) as f:
        ...
        for _ in range(test_loader.len):
            data, input_data, input_mask, positional, label = (
                test_loader.__load_next__()
            )
            # print(label)
            max_probs = []
            pos = 0
...
def validate(model, lossFunction, dev_loader):
    ...
    for _ in range(dev_loader.len):
        seed = random.randint(1, 1000000)
        torch.manual_seed(seed)
        data, input_data, input_mask, positional, answers = dev_loader.__load_next__()
...
def setup():
    ...
    parser.add_argument(
        "--train_path",
        type=str,
        default="data/cola_public/tokenized_splits/in_domain_train_split.tsv",
    )
    parser.add_argument(
        "--dev_path",
        type=str,
        default="data/cola_public/tokenized_splits/in_domain_val_split.tsv",
    )
    parser.add_argument(
        "--ood_val_path",
        type=str,
        default="data/cola_public/tokenized/out_of_domain_dev.tsv",
    )
    parser.add_argument(
        "--in_test_path",
        type=str,

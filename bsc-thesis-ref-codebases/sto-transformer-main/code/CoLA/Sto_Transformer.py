...
class Embeddings(nn.Module):
    def __init__(
        ...
        self.use_elmo = use_elmo  # TODO: What is ELMo? Embeddings from Language Models

        self.token_embeds = nn.Embedding(
            vocab_size, h_size, padding_idx=0
        )  # Eeach token in the vocabulary is embedded using a vector with size `h_size`.
        # h_size is default to be 128.
        self.pos_embeds = nn.Embedding(
            max_len, h_size + use_elmo * 1024
        )  # use_elmo is default to be True. --> h_size + use_elmo = 1152
        ...
        self.project = nn.Linear(
            use_elmo * 1024 + h_size, h_attn_size
        )  # `h_attn_size` is default to be 512 - argument `hidden_size` in `train.py`.
        self.gpu_id = gpu_id

    def forward(self, input, pos, data=None):
        if self.use_elmo:
            character_ids = (
                batch_to_ids(data).cuda(self.gpu_id)
                if self.gpu_id > -1
                else batch_to_ids(data)
            )
            rep = self.elmo(character_ids)["elmo_representations"][
                0
            ...
            rep = torch.cat(
                [rep, input], dim=-1
            )  # Shape: (batch_size, sequence_length, 1152)
        else:
            rep = self.token_embeds(input)  # Shape: (batch_size, sequence_length, 128)

        pos = self.pos_embeds(
            pos
        )  # Kích thước mỗi vector positional embedding phụ thuộc vào việc có dùng ELMo hay không.

        output = self.layer_norm(
            rep + pos
        )  # Tổng hợp thông tin của rep và pos bằng phép cộng rồi áp dụng LayerNorm lên kết quả.
        output = self.project(
            output
        )  # Biến đổi tuyến tính các vectơ biểu diễn về không gian 512 chiều để chuẩn bị tính toán attention.
...
class StoSelfAttention(nn.Module):
    def __init__(self, h_size, n_heads, prob_attn, prob_h, tau2):
        ...
        self.n_heads = n_heads  # Default to be 8
        self.h_size = h_size  # Default to be 512
        self.head_dim = self.h_size // self.n_heads  # Default to be 512 / 8 = 64

        self.query = nn.Linear(h_size, h_size)  # Shape: (512,512)
        self.key = nn.Linear(h_size, h_size)  # Shape: (512,512)
        self.value = nn.Linear(h_size, h_size)  # Shape: (512,512)
    ...
    def forward(self, input, input_mask):
        qq = self.query(input)  # Shape: (batch_size, sequence_length, 512)
        kk = self.key(input)  # Shape: (batch_size, sequence_length, 512)
        vv = self.value(input)  # Shape: (batch_size, sequence_length, 512)

        # Python will automatically infer `-1` into `sequence_length`.
        qq = qq.view(
            input.shape[0], -1, self.n_heads, self.head_dim
        )  # Shape: (batch_size, sequence_length, 8, 64)
        kk = kk.view(
            input.shape[0], -1, self.n_heads, self.head_dim
        )  # Shape: (batch_size, sequence_length, 8, 64)
        ...
        qq = qq.transpose(1, 2)  # Shape: (batch_size, 8, sequence_length, 64)
        kk = kk.transpose(1, 2)  # Shape: (batch_size, 8, sequence_length, 64)
        vv = vv.transpose(1, 2)  # Shape: (batch_size, 8, sequence_length, 64)

        interact = torch.matmul(
            qq, kk.transpose(-1, -2)
        )  # Shape: (batch_size, 8, sequence_length, sequence_length)
        ...
        mask_1 = (
            ...
            .unsqueeze(1)
            .expand(-1, self.n_heads, -1, input.shape[1])
        )
        mask_2 = (
            input_mask.unsqueeze(1)
            .unsqueeze(1)
            .expand(-1, self.n_heads, input.shape[1], -1)
        )
    ...
    def __init__(self, h_size, n_heads, prob_attn, prob_h, tau1, tau2, n_centroids):
        ...
        self.centroid = torch.nn.Parameter(
            torch.nn.init.uniform_(
                torch.empty(self.head_dim, self.n_centroids), a=-0.5, b=0.5
            ),
            requires_grad=True,
        )
...
class Layer(nn.Module):
    def __init__(
        ...
        if dual:
            self.attn = DualStoSelfAttention(
                h_size, n_heads, prob_attn, prob_h, tau1, tau2, centroids
            )
        else:

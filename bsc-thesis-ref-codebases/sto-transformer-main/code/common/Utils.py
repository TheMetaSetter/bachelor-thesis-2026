...
def generate_square_subsequent_mask(sz):
    ...
    mask = (
        mask.float()
        .masked_fill(~mask, neginf(torch.float32))
...
def build_copy_things(id, vocab2id, copy_source, output):
    ...
    dyn_output_tensor = torch.tensor(
        [dyn_vocab2id.get(w) if w in dyn_vocab2id else 0 for w in output],
        requires_grad=False,
    ).long()
    source_map_tensor = torch.tensor(
        [dyn_vocab2id.get(w) for w in copy_source], requires_grad=False
    )
    ...
    for i in range(len(dyn_id2vocab)):
        ...
        if dyn_id2vocab[i] in vocab2id:
            vocab_overlap.append(0.0)
        else:
            vocab_overlap.append(1.0)
...
def topk(gen_output, k=5, PAD=None, BOS=None, UNK=None):
    ...
    if k > 1:
        values, indices = torch.topk(
            gen_output, k, dim=1, largest=True, sorted=True, out=None
        )
    else:
...
def prepare_embeddings(emb_text_filepath):
    ...
    vectors = bcolz.carray(
        vectors[1:].reshape((idx, -1)), rootdir=emb_text_filepath + ".dat", mode="w"
    )
    vectors.flush()
...
def new_tensor(array, requires_grad=False):
    if torch.cuda.is_available():
        tensor = torch.tensor(
            array, device=torch.cuda.current_device(), requires_grad=requires_grad
        )
    else:
...
def gru_forward(gru, input, lengths, state=None, batch_first=True):
    ...
    outputs, output_lengths = torch.nn.utils.rnn.pad_packed_sequence(
        outputs, batch_first=batch_first, total_length=total_length
...
def merge1D(sequences, max_len=None, pad_value=None):
    ...
    if pad_value is None:
        padded_seqs = torch.zeros(len(sequences), max_len, requires_grad=False).type_as(
            sequences[0]
        )
    else:
        padded_seqs = torch.full(
            (len(sequences), max_len), pad_value, requires_grad=False
        ).type_as(sequences[0])

...
class SelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        key_out = torch.einsum(
            "nqhd,nkhd->nhqk", [Q, K]
        )  # batchx heads x query_len, key_len
...
class StoSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        key_out = torch.einsum(
            "nqhd,nkhd->nhqk", [Q, K]
        )  # batchx heads x query_sen_len, key_sen_len
...
class StoSelfDualAttention(nn.Module):
    def __init__(
        ...
        self.centroid = torch.nn.Parameter(
            init_function(torch.empty(self.head_dim, k_centroid), a=-0.5, b=0.5),
            requires_grad=True,
        )
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, value, key, query, mask):
        ...
        )  # [batch_size, sentence_len, emb_dim]
...
class StoTransformerBlock(nn.Module):
    def __init__(
        ...
        if direction == 1:
            ...
            self.attention = StoSelfDualAttention(
                emb_dim, heads, tau1, tau2, k_centroid
            )
    ...
    def forward(self, value, key, query, mask):
        attention = self.attention(
            query, key, value, mask
        )  # attention [batch_size, sentence_len, emb_dim]
        ...
        )  # query [batch_size, sentence_len, emb_dim]
        forward = self.feed_forward(x)
        out = self.dropout(self.norm2(forward + x))
        return out

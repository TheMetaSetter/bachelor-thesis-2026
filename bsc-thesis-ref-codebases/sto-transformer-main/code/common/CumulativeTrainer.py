...
class CumulativeTrainer(object):
    ...
    def train_epoch(
        ...
        for j, data in enumerate(train_loader, 0):
            ...
            bloss = self.train_batch(
                epoch, data, method=method, optimizer=optimizer, scheduler=scheduler
            )
    ...
    def predict(self, method, dataset, collate_fn, batch_size, epoch):
        ...
        with torch.no_grad():
            ...
            print(
                ...
                "Epoch=%s" % epoch,
                "Batch=%s" % batch_size,
                "DataLoaderLen=%s" % len(test_loader),
            )

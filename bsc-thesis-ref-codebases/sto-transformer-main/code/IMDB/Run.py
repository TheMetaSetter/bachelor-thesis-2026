...
def train(args, run_id=args.local_rank):
    ...
    model_bp_count = (args.n_epoch * train_size) / (
        args.n_gpu * args.train_batch_size * args.accumulation_steps
    )  # global_batch_step
    ...
    if args.warmup > 0:
        model_scheduler = get_cosine_with_hard_restarts_schedule_with_warmup(
            model_optimizer,
            round(args.warmup * model_bp_count),
    ...
    for i in range(args.train_epoch_start, args.n_epoch):
        train_loss_list = model_trainer.train_epoch(
            "train",
            train_dataset,
            collate_fn,
        ...
        dev_list_output, dev_loss_list = model_trainer.predict(
            "infer", valid_dataset, collate_fn, args.test_batch_size, i
        )
        test_list_output, test_loss_list = model_trainer.predict(
            "infer", test_dataset, collate_fn, args.test_batch_size, i
        )
        test_ood_list_output, test_ood_loss_list = model_trainer.predict(
            "infer", test_ood_dataset, collate_fn, args.test_batch_size, i
        )
        ...
        writer.add_scalars(
            main_tag="Loss_%s" % run_id,
            tag_scalar_dict={
                "Train": np.mean(train_loss_list),
        ...
        if dev_acc > max_dev_acc:
            max_epoch, max_dev_acc, report_test_acc, report_test_ood_acc = (
                i,
                dev_acc,
                test_acc,
                test_ood_acc,
            )
            model_trainer.serialize(
                max_epoch,
                output_path=OUTPUT_MODEL_DIR,
                use_multiple_gpu=True if args.n_gpu > 1 else False,
            )
            print(
                "Save model: rank=%s, max_epoch=%s, max_dev_acc=%s, report_test_acc=%s,\n path=%s"
                % (args.local_rank, max_epoch, max_dev_acc, test_acc, OUTPUT_MODEL_DIR)
            )
        else:
            patience -= 1
    ...
    print(
        "run_id=%s, max_epoch=%s, max_dev_acc=%s, report_test_acc=%s"
        % (args.local_rank, max_epoch, max_dev_acc, report_test_acc)
    )
    torch.save(
        dev_list_output,
        os.path.join(OUTPUT_RESULT_DIR, "valid.%s.%s" % (max_epoch, args.local_rank)),
    )
    torch.save(
        test_list_output,
        os.path.join(OUTPUT_RESULT_DIR, "test.%s.%s" % (max_epoch, args.local_rank)),
    )
    torch.save(
        test_ood_list_output,
        os.path.join(
            OUTPUT_RESULT_DIR, "test_ood.%s.%s" % (max_epoch, args.local_rank)
        ),
    )
...
def test_max_epoch(args, run_id=args.local_rank, epoch=None):
    ...
    if epoch is not None:
        ...
        if os.path.exists(file):
            ...
            dev_list_output, _ = model_trainer.predict(
                "infer", valid_dataset, collate_fn, args.test_batch_size, epoch
            )
            test_list_output, _ = model_trainer.predict(
                "infer", test_dataset, collate_fn, args.test_batch_size, epoch
            )
            test_ood_list_output, _ = model_trainer.predict(
                "infer", test_ood_dataset, collate_fn, args.test_batch_size, epoch
            )
            ...
            print(
                "run_id=%s, max_epoch=%s, max_dev_acc=%s, report_test_acc=%s, report_test_ood_acc=%s"
                % (run_id, epoch, dev_acc, test_acc, test_ood_acc)
            )
...
def test(args, run_id=args.local_rank):
    ...
    print(
        "run_id=%s, max_epoch=%s, max_dev_acc=%s, report_test_acc=%s"
        % (run_id, max_epoch, max_dev_acc, report_test_acc)
    )
    torch.save(
        dev_list_output,
        os.path.join(OUTPUT_RESULT_DIR, "valid.%s.%s" % (max_epoch, args.local_rank)),
...
if __name__ == "__main__":
    ...
    # 1. Load vocab
    vocab_path = "%s/%s.%s.vocab-%s.pkl" % (
        DATA_DIR,
        args.model_name,
        args.model_type.split("-")[0],
        args.train_valid_split_ratio,
    )
    ...
    if args.mode != "pre":
        ...
        all_test_ood_sample_tensor = wrap_dataset(
            create_ood_torchtext_dataset(), vocab, "test_ood"
        )
    ...
    if args.mode == "pre":
        ...
        for run_i in range(1, args.n_run):
            ...
            set_random_seed()
            _, test_acc, test_ood_acc = test_max_epoch(
                args, run_id=run_i, epoch=max_epoch
            )
        ...
        print(test_acc_list)
        print(
            "Uncertain: max_epoch=%s, mean_test_acc=%s, std_test_acc=%s, mean_test_ood_acc=%s, std_test_ood_acc=%s"
            % (
                max_epoch,
                np.mean(test_acc_list),
                np.std(test_acc_list),
                np.mean(test_ood_acc_list),
                np.std(test_ood_acc_list),
            )
        )

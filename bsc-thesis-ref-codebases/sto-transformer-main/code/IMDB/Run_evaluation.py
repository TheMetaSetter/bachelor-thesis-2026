...
def load_n_result(output_result_dir):
    ...
    for file in files:
        ...
        list_output = torch.load(
            os.path.join(output_result_dir, file), map_location=torch.device("cpu")
        )
        # print('Load:', file)
        for golden_data, pred_output in tqdm(
            list_output, disable=False if args.debug else True
        ):  # each output from each gpu
            try:
...
def show_result(all_acc, output_result_dir, save_figure=1):
    ...
    print(
        "Summary: Epoch=%s, Valid_Acc=%.4f %%, Test_Acc=%.4f %%"
...
def eval_from_files(output_result_dir):
    ...
    for eval_mode in all_label:
        for ep in all_label[eval_mode]:
            ...
            acc = calculate_acc(
                output=torch.cat(all_pred[eval_mode][ep]),

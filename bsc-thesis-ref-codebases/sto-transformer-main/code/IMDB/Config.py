...
parser.add_argument(
    ...
    type=str,
    default="IMDB",
    help="Assign the subdirectory of the running model",
)
parser.add_argument(
    "--exp_name",
    type=str,
    default="default",
    help="extra tag to memorize experimental setup",
)
...
parser.add_argument(
    "--mode", type=str, default="train", help="pre/train/test/eval/uncertain"
)
parser.add_argument(
    "--debug",
    type=int,
    default=0,
    help="to speed up running a result during debugging phase",
)
parser.add_argument(
    "--early_stop",
    type=int,
    default=30,
    help="if loss does not decrease N epoch then stop training.",
)
...
parser.add_argument(
    ...
    type=int,
    default=-1,
    help="test one epoch; -1 means test latest epoch",
)
parser.add_argument(
    "--device",
    type=object,
...
parser.add_argument(
    "--sentence_len", type=int, default=500, help="Maximum sequence length"
)  # TF
parser.add_argument("--train_epoch_start", type=int, default=0)
parser.add_argument("--infer_epoch_start", type=int, default=0)  #
parser.add_argument(
    "--warmup", type=float, default=-1, help="Percentage of warm up steps"
)
parser.add_argument(
    "--max_grad_norm", type=int, default=10, help="Clip gradients to this norm."
)  # 1, 10.0, 40.0
parser.add_argument(
    "--weight_decay", type=float, default=5e-4, help="Cweight decay."
)  # 1, 10.0, 40.0
...
parser.add_argument(
    ...
    type=int,
    default=-1,
    help="Number of samples; -1: do not sample.",
)
# transformers
parser.add_argument("--n_layer", type=int, default=1)
parser.add_argument("--emb_dim", type=int, default=128)
parser.add_argument(
    "--output_dim", type=int, default=2, help="Number of classes."
)  # TF
...
parser.add_argument(
    ...
    type=int,
    default=2,
    help="Number of centroid of attentions in transformers.",
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="use only when to specify a model path",
)
...
# ** construct macros **
FLAG_OUTPUT = (
    args.local_rank is None or args.local_rank == 0
)  # for multiple gpu output only once
SETTING = "%s_%s_%s" % (
    args.model_name,
    args.model_type,
    args.exp_name,
)  # e.g., IMDB_tf_default/20210415_123456
...
if args.mode == "test":
    ...
    latest_model_path = max(
        [os.path.join(OUTPUT_MODEL_DIR, d) for d in os.listdir(OUTPUT_MODEL_DIR)],
        key=os.path.getmtime,
    )
    args.test_epoch = (
        int(re.split("/|\.", latest_model_path)[-2])
        if args.test_epoch == -1
        else args.test_epoch
    )

...
def get_imdb(
    ...
    if model_type == "tf":  # transformer
        ...
        LABEL = t_data.Field(
            sequential=False, use_vocab=False, unk_token=None, pad_token=None
        )
    elif model_type == "bert":  # bert
        tokenizer = BertTokenizer.from_pretrained(pretrained_model)
        sentence_len = min(
            max_length, tokenizer.max_model_input_sizes[pretrained_model]
        )
        TEXT = t_data.Field(
            batch_first=True,
            use_vocab=False,
    ...
    _train_dataset, _test_dataset = datasets.IMDB.splits(
        TEXT, LABEL, root=data_root_dir
    )
    _train_dataset, _valid_dataset = _train_dataset.split(
        args.train_valid_split_ratio, random_state=random.seed(SEED)
    )
    ...
    if args.model_type == "tf":
        TEXT.build_vocab(
            ...
            vectors=GloVe(
                ...
                dim=300,
                max_vectors=10000,
                cache="%s/vectors_cache" % data_root_dir,
            ),
        )
...
class IMDBDataset(torch.utils.data.Dataset):
    ...
    def load(self):
        for id in tqdm(
            ...
            desc="sample",
            disable=False if args.debug else True,
        ):
            ...
            sent = (
                [CLS_WORD]
                + text
                + [PAD_WORD] * (self.sentence_len - len(text) - 2)
                + [EOS_WORD]
            )  # padding a sentence
            # sent = sample.text
            # Numericalization
            text_tensor = torch.tensor(
                [self.vocab2id.get(w, self.vocab2id[UNK_WORD]) for w in sent]
            ).long()
...
class CRDataset(torch.utils.data.Dataset):
    def __init__(
        self, samples, vocab, sentence_len=args.sentence_len, n=1e10, sample_tensor=None
    ):
        """
        Given a torchtext_dataset and vocab object as an input, wrap a torch_dataset object
        """
...
def wrap_dataset(
    ...
    vocab,
    mode,
    ratio=args.train_valid_split_ratio,
    ...
    dataset_path = "%s/%s.%s.%s-%s.pkl" % (
        data_dir,
        model_name,
        model_type,
        mode,
        ratio,
    )  # e.g., data/imdb/IMDB.tf.train.pkl
...
def prepare_dataset(args):
    ...
    torchtext_train_dataset, torchtext_valid_dataset, torchtext_test_dataset, vocab = (
        get_imdb(data_root_dir=args.data_root_dir)
    )
    ...
    test_ood_sample_tensor = wrap_dataset(
        torchtext_test_ood_dataset, vocab, mode="test_ood"
    )

    # 3. Iter and see train dataset
    train_dataset = IMDBDataset(
        torchtext_train_dataset, vocab, sample_tensor=train_sample_tensor
    )

...
class IMDB(nn.Module):
    def __init__(self, emb_dim, output_dim, vocab, dropout=0.1):
        ...
        self.transformer_encoder = StoTransformerEncoder(
            src_vocab_size=self.vocab_size,
            src_pad_idx=0,
            emb_dim=self.emb_dim,
            num_layers=args.n_layer,
            forward_expansion=args.n_fe,
            heads=args.n_head,
    ...
    def transformers_encode(self, token_seq):  # batch_numseq_seqlen
        ...
        seq_enc = seq_enc.reshape(
            batch_size, num_seq, seq_len, -1
        )  # [12, 20, 4] -> [3, 4, 20, 4]
    ...
    def sentiment_classification_v0(self, query):
        query_enc, query_mask = self.transformers_encode(
            query
        )  # [batch_size, 1, seq_len] -> [batch_size, 1, seq_len, n_dim]
        query_enc = query_enc.squeeze(1)  # [batch_size, seq_len, n_dim]
        query_mask = query_mask.squeeze(1)  # [batch_size, seq_len]
        q = universal_sentence_embedding(
            query_enc, query_mask
        )  # sum [batch_size, n_dim]
    ...
    def do_infer(self, data):
        ...
        prediction = F.softmax(
            self.sentiment_classification(text), dim=-1

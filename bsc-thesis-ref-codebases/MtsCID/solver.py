...
class Solver(object):
    ...
    def train(self):
        """
        Hàm này chứa vòng lặp train chính của phương pháp.
        ...
        # Sử dụng kỹ thuật early stopping để tránh hiện tượng overfit
        early_stopping = OneEarlyStopping(
            patience=self.patience, verbose=True, dataset_name=self.dataset
        )

        # Số lượng training step trong mỗi epoch
        # là số lượng batch trong dataset.
        train_steps = len(self.train_loader)

        # Bắt đầu tính thời gian train với
        # thời gian train là tổng thời gian chạy tất cả các epoch.
        training_start_time = time_now = time.time()

        # Đối với mỗi epoch trong dataset
        for epoch in tqdm(range(self.num_epochs)):
            # Khởi tạo các biến tạm để lưu thống kê của các epoch
            iter_count = 0
            loss_list = []
            rec_loss_list = []
            intra_loss_list = []
            ...
            # Bắt đầu tính thời gian chạy epoch này
            epoch_time = time.time()

            # Đặt tất cả các module bên trong mô hình trong chế độ training.
            # Đặc biệt là các module có sử dụng kỹ thuật dropout và batch normalization.
            # Trong giai đoạn training, dropout sẽ tắt một số nơ-ron để mô hình tránh bị overfit.
            # Còn batch norm tính toán mean và variance dựa trên từng channel trong batch dữ liệu hiện tại.
            # Trong giai đoạn evalution, dropout sẽ KHÔNG tắt nơ-ron nào cả. Còn batch norm sẽ sử dụng
            # mean và variance được tích luỹ trong quá trình training.
            self.model.train()  # Đây thực ra là gọi hàm nn.Module.train().

            # Đối với mỗi batch trong bộ dữ liệu
            for i, (input_data, labels) in enumerate(self.train_loader):
                # Reset các gradient trong bộ nhớ về lại 0.
                self.optimizer.zero_grad()

                # Chuẩn bị chạy thêm một step nữa
                # với mỗi step tương ứng với một batch.
                iter_count += 1

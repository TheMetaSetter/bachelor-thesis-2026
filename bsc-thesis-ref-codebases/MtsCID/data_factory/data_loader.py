...
class SMDSegLoader(Dataset):
    def __init__(self, data_path, win_size, step, mode="train"):
        ...
        self.mode = mode
        self.step = step  # Mặc định là 100 (time-steps)
        self.win_size = win_size  # Mặc định là 100 (time-steps)
        self.scaler = StandardScaler()
    ...
    def __len__(self):
        if self.mode == "train":
            return (
                self.train.shape[0] - self.win_size
            ) // self.step + 1  # Số lượng sample trong dataset
        elif self.mode == "test":
            return (self.test.shape[0] - self.win_size) // self.step + 1
        else:
            return (self.train.shape[0] - self.win_size) // self.step + 1
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            return np.float32(self.train[index : index + self.win_size]), np.float32(
                np.zeros(self.win_size)
            )  # Mỗi lần duyệt dataloader sẽ trả về một sample trong dataset.
        elif self.mode == "test":
            return np.float32(self.test[index : index + self.win_size]), np.float32(
                self.test_labels[index : index + self.win_size]
            )

...
def test_online_benchmark_validates_reference_checkpoint_hash_before_streaming(
    ...
    reference_checkpoint_path.write_bytes(b"reference-checkpoint")
    ...
    monkeypatch.setattr(
        "src.engine.online_tta.online_engine_run._build_model_from_experiment_config",
        lambda experiment_config: _SpyOnlineBenchmarkModel(),
    )
    monkeypatch.setattr(
        "src.engine.online_tta.online_engine_run._build_optimizer_from_experiment_config",
        lambda model, experiment_config: torch.optim.Adam(
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: ([{"online/step": 1}], [{"entity_id": "machine-1-6"}]),
    )

    context = _build_runtime_online_context(
        experiment_config={
            "experiment_name": "online-hash-spy",
            "seed": 7,
            "device": "cpu",
            "output_dir": str(tmp_path / "outputs"),
            "checkpoint_dir": str(tmp_path / "checkpoints"),

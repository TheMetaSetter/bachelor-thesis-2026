...
def test_execution_selects_a_distinct_artifact_for_each_entity(monkeypatch) -> None:
    ...
    def run_sequence(**kwargs):
        seen.append((kwargs["sequence"]["meta"]["entity_id"], kwargs["threshold_value"]))
        return [], []
    ...
    context = {
        "data_bundle": {"scaled_sequences": {"test": [
            {"meta": {"entity_id": "m1"}}, {"meta": {"entity_id": "m2"}}
        ]}},
        "threshold_artifacts": {"m1": _artifact("m1", 1.0), "m2": _artifact("m2", 9.0)},
        "model": object(), "optimizer": object(), "online_variant": "A2",
        "batch_size": 1, "view_noise_std": 0.0, "view_dropout_probability": 0.0,
        "device": "cpu", "verification_buffer": object(), "ttl_buffer": object(),
        "hard_old_guard": object(), "signature_history": [], "max_online_steps": 0,
    }

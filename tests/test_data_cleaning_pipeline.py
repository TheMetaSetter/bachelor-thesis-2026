...
def test_sequence_cleaning_pipeline_preserves_shapes_and_can_annotate_metadata() -> None:
    ...
    cleaned_sequences = cleaning_pipeline.transform_splits({"train": [parsed_sequences["train"][0]]})
    first_cleaned_sequence = cleaned_sequences["train"][0]

    assert first_cleaned_sequence["x"].shape == parsed_sequences["train"][0]["x"].shape
    assert first_cleaned_sequence["point_labels"].shape == parsed_sequences["train"][0]["point_labels"].shape
...
def test_public_data_api_can_annotate_cleaning_metadata() -> None:
    ...
    assert public_bundle.raw_sequences["train"][0]["meta"]["cleaning"] == "validated_sequence_contract"

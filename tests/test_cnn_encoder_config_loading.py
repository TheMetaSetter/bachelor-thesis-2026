    ...
    experiment_path.write_text(
        "\n".join(
            [
                ...
                "    monitor_metric: val_realistic_vus_pr",
                "    factor: 0.5",
                "    patience: 5",
                "    threshold: 0.0001",
                "    threshold_mode: rel",
...
def test_mlp_and_cnn_model_configs_load_and_surface_encoder_family(
    ...
    baseline_experiment_path = _write_experiment_config(
        ...
        model_config_path=str(REPO_ROOT / "configs/model/redlamp_mlp_baseline.yaml"),
    )
    ...
    baseline_model = RedLampMLPBaseline(**baseline_config["model"])

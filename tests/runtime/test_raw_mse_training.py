...
def build_trainer(tmp_path, model=None):
    ...
    return Trainer(model, torch.optim.Adam(model.parameters()), None, None,
                   CheckpointManager(tmp_path / "checkpoints"),
                   ExperimentLogger(tmp_path / "logs", {"experiment_name": "raw"},
...
def test_trainer_attaches_raw_train_scaler_and_scores_mc_errors(tmp_path):
    ...
    trainer._configure_reconstruction_context({"reconstruction_loss_space": "raw_input"}, scaler_state())
    samples = torch.tensor([[[[1., 0.]], [[-1., 0.]]]])
    step = {"batch": {"x": torch.zeros(1, 1, 2)}, "outputs": {
        "point_scores": torch.tensor([[999.]]), "recon": samples.mean(dim=1),
        "aux": {"stochastic_query": {"reconstruction_samples": samples}}}}
    torch.testing.assert_close(trainer._validation_point_scores(step), torch.tensor([[2.]]))
...
def test_synthetic_metrics_use_supplied_clean_threshold(tmp_path):
    ...
    trainer._configure_reconstruction_context({"reconstruction_loss_space": "raw_input"}, scaler_state())
    class Dataset:
        sequences = [{"x": torch.zeros(4, 2), "point_labels": torch.zeros(4),
                      "meta": {"entity_id": "toy"}}]
    class Loader:
        dataset = Dataset()
    payloads = [{"meta": [{"entity_id": "toy", "start_index": 0, "end_index": 4}],
                 "point_scores": torch.tensor([[1., 2., 8., 9.]]),
                 "point_labels": torch.tensor([[0, 0, 1, 1]])}]
    metrics = trainer._aggregate_reconstructed_pointwise_metrics(
        data_loader=Loader(), batch_payloads=payloads, stage_name="val_synth", threshold=3.)
    assert metrics["val_synth_threshold"] == 3.
    assert metrics["val_synth_f1_pointwise"] == 1.


def test_raw_training_rejects_sigmoid_evaluation_config(tmp_path):
    trainer = build_trainer(tmp_path)
    with pytest.raises(ValueError, match="identity"):
        trainer._configure_reconstruction_context({"reconstruction_loss_space": "raw_input",
            "evaluation": {"point_score_transform": "sigmoid"}}, scaler_state())
...
def test_cli_raw_evaluation_fits_threshold_on_clean_loader():
    ...
    from tests.evaluation.test_raw_input_mse_scores import (
        _RawEvaluationModel, _RawEvaluationLoader, _fit_one_feature_scaler,
    )
    result = evaluate.evaluate_raw_checkpoint(
        Evaluator(device="cpu"), _RawEvaluationModel(),
    ...
    assert result["metrics"]["threshold"] == pytest.approx(.25)
    assert result["metrics"]["threshold_source"] == "clean_validation_quantile"
    assert result["records"][0]["point_scores"].tolist() == [.25, 0., .25]
...
def test_raw_checkpoint_threshold_metadata_names_clean_validation():
    ...
    metadata = build_checkpoint_evaluation_metadata(
        ...
        epoch_metrics={"val_score_space": "raw_input", "val_threshold": 3.,
                       "val_synth_threshold": 9., "val_synth_vus_pr": .8},
        base_extra_state=None,
    )
    assert metadata["evaluation_threshold"] == 3.
    assert metadata["evaluation_threshold_source"] == "clean_validation_quantile"
    assert metadata["checkpoint_monitor_value"] == .8

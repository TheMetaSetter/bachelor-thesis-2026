...
def build_model():
    return ThesisMultitaskModel(
        input_dim=2, window_size=4, encoder_dim=8, hidden_dim=4,
        num_classes=2, continuous_num_prototypes=2, discrete_codebook_size=2,
        training_phase="stage_a_multitask_pretraining",
        use_synthetic_augmentation=False, use_synthetic_validation=False,
        reconstruction_normal_only=True, enable_score_loss=True,
...
def scaler_state():
    return {
        "epsilon": 0.001, "feature_mean": torch.tensor([10., 5.]),
        "feature_std": torch.tensor([2., 0.]),
...
def test_raw_masked_loss_and_gradient_preserve_inactive_sensor_units():
    ...
    reconstruction = torch.tensor([[[1., 2.], [9., 9.]]], requires_grad=True)
    batch = {"x": torch.zeros_like(reconstruction),
             "synthetic_anomaly_mask": torch.tensor([[0, 1]])}
    loss = model._compute_reconstruction_loss({"recon": reconstruction}, batch)
    assert loss.item() == pytest.approx(4.)  # (2**2 + 2**2) / 2
    loss.backward()
    torch.testing.assert_close(reconstruction.grad, torch.tensor([[[4., 2.], [0., 0.]]]))
    diagnostics = model._compute_reconstruction_diagnostics({"recon": reconstruction}, batch)
...
def test_raw_loss_averages_sample_errors_before_mc_reduction():
    ...
    outputs = {"recon": samples.mean(dim=1),
               "aux": {"stochastic_query": {"reconstruction_samples": samples}}}
    assert model._compute_reconstruction_loss(outputs, {"x": torch.zeros(1, 1, 2)}).item() == 2.
    assert model._compute_reconstruction_diagnostics(outputs, {"x": torch.zeros(1, 1, 2)})[
...
def test_raw_point_supervision_uses_sensor_weighted_mse():
    model = build_model()
    model.configure_reconstruction_loss("raw_input", scaler_state())
    outputs = {"recon": torch.tensor([[[1., 0.], [0., 1.], [1., 1.], [2., 0.]]])}
    batch = {"x": torch.zeros(1, 4, 2), "synthetic_anomaly_mask": torch.tensor([[0, 0, 1, 1]])}
    loss, _ = model._compute_point_score_loss(outputs, batch)
    # Raw point MSE=[2,.5,2.5,8]; normal mean=1.25, std=.75.
    logits = torch.tensor([1., -1., 5./3., 9.])
    expected = torch.nn.functional.binary_cross_entropy_with_logits(logits, torch.tensor([0., 0., 1., 1.]))
...
def test_checkpoint_restores_raw_loss_context(tmp_path):
    ...
    path = manager.save_checkpoint("best.pt", model, None, None, scaler_state(),
                                   {"reconstruction_loss_space": "raw_input"}, 1, [])
    restored = build_model()
    manager.load_checkpoint(path, restored)
    assert restored.reconstruction_loss_space == "raw_input"
    assert restored._compute_reconstruction_loss(
...
def test_inverse_transform_retains_float64_autograd():
    ...
    torch.testing.assert_close(values.grad, torch.tensor([[2., 1.]], dtype=torch.float64))


@pytest.mark.parametrize("enable_score_loss", [False, True])
def test_raw_stage_a_variants_forward_backward(enable_score_loss):
    from tests.models.test_thesis_multitask_point_score_loss import _build_model, _build_batch

    model = _build_model(enable_score_loss)
    model.configure_reconstruction_loss("raw_input", {
        "epsilon": .001, "feature_mean": torch.zeros(38),
        "feature_std": torch.full((38,), 2.),
    ...
    expected = errors[normal].mean() if model.reconstruction_normal_only else errors.mean()

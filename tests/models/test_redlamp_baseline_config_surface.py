...
def test_validate_experiment_config_rejects_legacy_val_realistic_task_keys() -> None:
    experiment_config = {
        "experiment_name": "redlamp-baseline-config-legacy-task-keys",
        "seed": 7,
        "device": "cpu",
        "output_dir": "outputs/redlamp-baseline-config-legacy-task-keys",
        "checkpoint_dir": "outputs/redlamp-baseline-config-legacy-task-keys/checkpoints",
        "data": {
            "dataset_name": "smd",
            "root_dir": "data/ServerMachineDataset",
            "window_size": 20,
    ...
    with pytest.raises(ValueError, match="Unknown task config keys"):
        validate_experiment_config(experiment_config)

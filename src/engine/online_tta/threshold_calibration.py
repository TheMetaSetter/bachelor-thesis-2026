...
def compute_input_window_score(
    reconstruction: torch.Tensor, input_window: torch.Tensor
) -> torch.Tensor:
    if reconstruction.shape != input_window.shape or reconstruction.ndim != 3:
        raise ValueError(
            "reconstruction and input_window must both have shape [B, L, C]"
        )
    ...
    for key in ("latent_window_score", "memory_score", "latent_score"):
        ...
        if isinstance(value, torch.Tensor):
            return (
                value
                if value.ndim == 1
                else value.reshape(value.shape[0], -1).mean(dim=1)
            )
    raise KeyError(
        "online model outputs must expose an explicit latent or memory score"
    )


def aggregate_endpoint_ewma(
    current: float,
    previous: float | None,
...
def quantile_threshold(
    scores: torch.Tensor | Sequence[float], quantile: float
) -> float:

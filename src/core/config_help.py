...
def build_config_help_text(command_name: str) -> str:
    return dedent(
        ...
        5) Useful commands
          ...
          - Train:
              python scripts/train.py --experiment-config configs/experiment/baseline/smd__redlamp_mlp_baseline__redlamp-mlp-baseline-window20__w20__seed11__default.yaml
          - Evaluate:
              python scripts/evaluate.py --experiment-config configs/experiment/baseline/smd__redlamp_mlp_baseline__redlamp-mlp-baseline-window20__w20__seed11__default.yaml --checkpoint-path outputs/.../best.pt
        """
    ).strip()

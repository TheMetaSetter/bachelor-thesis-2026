...
"""Window-level KMeansAD baseline with clean-validation calibration.

₍^. .^₎⟆ Method flow

train sequence
  -> non-overlap tail windows
  -> KMeans fit on window vectors
  -> window distance scores
  -> point-level clean-validation calibration
"""
...
class KMeansADWindowBaseline(TraditionalBaselineProtocol):
    def __init__(
        ...
        *,
        ...
        self.calibration_: KMeansADCalibration | None = None

    def fit(self, train_sequence: np.ndarray) -> "KMeansADWindowBaseline":
    ...
    def _score_windows(self, query_sequence: np.ndarray) -> tuple[np.ndarray, int]:
        ...
        return distances.astype(np.float64, copy=False), int(
            np.asarray(query_sequence).shape[0]
        )
    ...
    def calibrate(self, clean_validation_sequence: np.ndarray) -> dict[str, Any]:
        ...
        self.calibration_ = KMeansADCalibration(

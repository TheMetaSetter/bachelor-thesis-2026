...
- The multitask training logic is located in `src/tasks/multitask_tsad_task.py`, not in `src/models/thesis_multitask.py`. The task file prepares batches, applies synthetic anomaly injection during training, computes reconstruction and classification losses, computes prototype regularization, and calculates classification accuracy.

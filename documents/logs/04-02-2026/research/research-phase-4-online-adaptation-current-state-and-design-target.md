...
- **Current repository state.** The configuration inventory now includes both the offline multitask family and the first online family: `configs/experiment/baseline/smd__thesis_multitask__multitask__w100__seed7__default.yaml`, `configs/experiment/smoke/smd__thesis_multitask__multitask-smoke__w100__seed7__smoke.yaml`, the explicit ablation variants under `configs/experiment/`, and `configs/experiment/baseline/smd__online_adaptation__online-adaptation__w100__seed7__default.yaml` together with `configs/model/online_adaptation.yaml` and `configs/task/online_adaptation.yaml`.

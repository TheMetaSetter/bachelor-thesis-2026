...
class DataEntity(object):
    def __init__(self,
                 ...
                 verbose: bool = False) -> 'DataEntity':
        
        # Load input arguments into attributes of the entity
        ## Load primary time-series data - main data
        self.Y = Y
        ...
        if X is None:   # If the user didn't pass any exogenous covariates
            ...
            assert num_exo_timesteps == self.num_timesteps, "The number of timesteps in the main data (Y) and optional/exogenous data (X) must be the same."
            
            self.X = X
            self.num_exogenous = num_exogenous
        
        ## Load labels
        if labels is None:  # If the user didn't pass any anomaly labels for the main data
            ...
            assert labels.shape[-1] == self.num_timesteps, "The number of timesteps in the main data (Y) and the labels array must be the same."
            self.labels = labels
        ...
        if mask is None:    # If the user didn't pass any anomaly mask
            ...
            assert mask.shape == Y.shape, "The main time-series data (Y) and the anomaly mask must have the same shape."
            self.mask = mask
...
class Dataset(object):
    def __init__(self,
        ...
        assert all([(entity.num_features == self.num_features) for entity in self.entities]), "All entities must have the same number of features."
        
        # Check if all entities have the same number of exogenous covariates
        assert all([(entity.num_exogenous == self.num_exogenous) for entity in self.entities]), "All entities must have the same number of exogenous covariates/variables."
